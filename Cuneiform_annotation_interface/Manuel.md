...
De la même façon, si vous cliquez sur un signe non annoté (non rouge), vous passez en mode Ajout.  
...
Si jamais les corrections que vous avez apportées ne vous conviennent pas, vous pouvez réinitialiser le signe sélectionné en appuyant sur le **⟳** rouge en haut à droite.  
...
## 4.b Réinitialiser les annotations faites sur une image  
Si vous appuyez sur ce bouton, toutes vos annotations sur l'image sélectionnée seront supprimées des sauvegardes. Cela permet d'alléger le poids du cache et aussi de ne pas avoir une liste trop longue de tablettes sauvegardées.  
